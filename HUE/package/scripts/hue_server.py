...
class HueServer(Script):
  ...
  def install(self, env):
    ...
    download_hue()
    Execute('mkdir -p /data/log/hue')
    Execute('chown -R hue:hue /data/log/hue')
  ...
  def start(self, env):
    ...
    self.stop(env)
    self.configure(env)
    Execute(format("{hue_bin_dir}/hue runcpserver >> {hue_log_file} 2>&1 &"),
  ...
  def stop(self, env):
    ...
    Execute ('ps -ef | grep runcherrypyserver | grep -v grep | awk  \'{print $2}\' | xargs -r kill -9', user=params.root, ignore_failures=True)
  ...
  def status(self, env):
    ...
    Execute ('ps -ef | grep runcherrypyserver | grep -v grep | awk \'{print $2}\' > ' + params.hue_server_pid_file, user=params.hue_user)
  ...
  def metastoresync(self, env):
    ...
    if params.metastore_db_flavor != 'sqlite3':
      Execute (format("{hue_bin_dir}/hue syncdb --noinput"), user=params.root)
      Execute (format("{hue_bin_dir}/hue migrate"), user=params.root)
    else:
